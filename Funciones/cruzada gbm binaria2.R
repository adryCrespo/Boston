...
###############################
#Cambios con respecto a cruzadagbmbin:
#   muestra accuracy y no tasa de fallos (1-accuracy)
#   muestra sensibilidad: nueva funcion sensibilidad 
#   sampling while resampling: 
#      cambio en el control de caret linea 48 sampling = "rose"
#      library(ROSE)
#
###############################

cruzadagbmbin2<-function(data=data,vardep="vardep",
  ...
  control<-trainControl(method = "repeatedcv",number=grupos,repeats=repe,
   savePredictions = "all",classProbs=TRUE,sampling = "rose") 
  ...
  medias<-preditest %>%
   group_by(Rep) %>%
   summarize(Accuracy=tasafallos(pred,obs))
  ...
  s1<-preditest %>%
    group_by(Rep) %>%
    summarize(sensitivity=sensibilidad(pred,obs))
